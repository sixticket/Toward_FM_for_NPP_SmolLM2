"""
모델 예측 → 시뮬레이터 검증 스크립트 (V7.1 Simple 1K)
- Phase 2 V7.1 Simple 1K 모델로 제어봉 파라미터 예측
...
# 모델 경로 (V7.1 Simple 1K!)
TRAINING_DIR = REPO_ROOT / "training"
PHASE1_MODEL_PATH = TRAINING_DIR / "models/smollm2_unsupervised_numeric_1k/final_model"
PHASE2_MODEL_PATH = TRAINING_DIR / "models/smollm2_supervised_lora_v7_numeric_simple_1k/final_model"  # V7.1 1K!
...
VALIDATION_DIR = SCRIPT_DIR / "validation_runs_v7_simple_1k"  # V7.1 1K 전용 디렉토리
...
def load_model():
    """Phase 2 V7.1 Simple 1K 모델 로드 (Phase 1 + LoRA)"""
    print("=" * 80)
    print("모델 로딩 중... (V7.1 Simple 1K)")
    print("=" * 80)
    ...
    # LoRA adapter 로드
    print(f"✓ Phase 2 LoRA (V7.1 Simple 1K): {PHASE2_MODEL_PATH}")
...
def run_validation_suite(model, tokenizer, num_cases=100):
    """여러 테스트 케이스 검증"""
    print("\n" + "🎯 " * 20)
    print(f"V7.1 Simple 1K 모델 → 시뮬레이터 검증 ({num_cases} 케이스)")
    print("🎯 " * 20)
    ...
    # V7.1 10K vs 1K 비교
    print(f"\n📊 V7.1 10K vs 1K 비교:")
    print(f"  V7.1 10K 파싱:        100.0%")
    print(f"  V7.1 1K 파싱:         {parsing_success / total * 100:.1f}%")
    if simulation_success > 0:
        print(f"  V7.1 10K 검증(±5%):   79.0%")
        print(f"  V7.1 1K 검증(±5%):    {validation_success_5 / simulation_success * 100:.1f}%")
        print(f"  V7.1 10K 검증(±10%):  94.0%")
        print(f"  V7.1 1K 검증(±10%):   {validation_success_10 / simulation_success * 100:.1f}%")
    ...
    results_file = VALIDATION_DIR / f"validation_results_v7_simple_1k_{num_cases}cases_{timestamp}.json"
...
def main():
    print("\n" + "=" * 80)
    print("V7.1 Simple 1K 모델 → 시뮬레이터 검증 도구")
    print("=" * 80)
    ...
    # 검증 케이스 수 선택
    print("\n📊 검증 케이스 수 선택:")
    print("  [1] 100개 (빠른 테스트, 15-25분)")
    print("  [2] 200개 (표준 테스트, 30-50분) ⭐ 추천")
    print("  [3] 500개 (상세 테스트, 1.5-2.5시간)")
    ...
    print("\n" + "✅ " * 20)
    print("V7.1 Simple 1K 검증 완료!")
    print("✅ " * 20)
    print("\n📌 1K 모델 특징:")
    print("  ✓ 빠른 학습 (Phase 1: ~1시간, Phase 2: ~2시간)")
    print("  ✓ 작은 데이터셋으로 개념 검증")
    print("  🎯 목표: 80-85% 검증 성공률")
